...
data = NSDQ100.option_chain(exp_date)
# print(data)
puts= data.puts[~data.puts.inTheMoney]
calls= data.calls[~data.calls.inTheMoney]
...
f = np.poly1d(poly)
fderiv = np.polyder(f,m=2)
impliedPDFVals = fderiv(new_K_vals)

fig, ax2 = plt.subplots(1, 1,figsize=(15,10))

## implied volatility curves
ax2.plot(new_K_vals, impliedPDFVals)
ax2.set_title('PDF of  ' + exp_date)
ax2.set_xlabel('$K$')
